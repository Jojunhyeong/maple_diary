...
1. GitHub Actions가 매주 토요일 05:17 KST에 공식 랭킹의 직업군별 상위·중간·심층 페이지를 나눠 조회합니다.
2. 주간 작업은 캐릭터의 OCID·직업·전투력만 서버 전용 Supabase 색인에 저장합니다. 장비 상세는 이 단계에서 조회하지 않습니다.
3. 사용자가 `비슷한 장비 검색`을 누르면 같은 직업의 캐릭터를 목표 전투력과 가까운 순서로 고릅니다. 공식 랭킹 API에는 전투력 범위 필터가 없어 서버 전용 색인을 사용합니다.
4. `직업 + 1,000만 전투력 구간`의 7일 캐시가 없을 때만 유효 캐릭터 30명의 장비를 수집합니다. 전투력과 같은 날짜에 실제 적용 중이던 장비만 사용하고, 드롭률·메소 획득량 잠재가 있으면 해당 캐릭터를 제외합니다.
