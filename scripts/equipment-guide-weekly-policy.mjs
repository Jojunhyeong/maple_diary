export function validateWeeklySnapshot(previous, next, expectedDate) {
  ...
  if (next.schemaVersion !== 2) throw new Error('가까운 전투력 표본 형식이 아닙니다.');
  const groups = stats => new Set(stats.map(row => `${row.job}:${row.cohortPower ?? row.combatPowerBucket}`));
  ...
  const previousUsesCohorts = (previous?.stats || []).some(row => Number.isFinite(row.cohortPower));
  if (previousUsesCohorts && retained < Math.ceil(previousGroups.size * 0.5)) throw new Error('기존 직업·전투력 표본의 절반 이상이 누락되었습니다.');
  for (const stat of next.stats) {
    if (stat.updatedAt !== expectedDate || !Number.isFinite(stat.cohortPower) || !Number.isFinite(stat.powerMin) || !Number.isFinite(stat.powerMax) || stat.powerMin > stat.powerMax || !Number.isSafeInteger(stat.sampleCount) || stat.sampleCount < 1 || stat.sampleCount > 50 || !stat.items?.length) throw new Error('전투력 표본 통계 검증 실패');
