...
- 직업 필터 없는 종합 랭킹의 여러 페이지에서 분산 추출 → OCID 조회 → 동일 날짜 전투력 조회 → 구간 내 캐릭터의 실제 착용 장비 조회.
...
- 초당 최대 약 4.5회 순차 조회, 429/5xx 제한 재시도. 실패 또는 SIGINT/SIGTERM 시 진행 상황을 저장하고 종료합니다. 20명마다 체크포인트를 저장하고 완료한 페이지와 행은 재실행 시 건너뜁니다. 같은 날짜로 재실행하면 완료한 캐릭터를 중복 집계하지 않습니다. 잘못된 API 키·파라미터·준비 중인 데이터 오류는 캐릭터 없음으로 처리하지 않고 중단합니다.
- `.cache/equipment-guide/collector.lock`으로 동시 실행을 막습니다. 강제 종료로 잠금이 남으면 파일에 기록된 PID가 실행 중인지 확인한 후에만 잠금을 삭제하세요.
- `.cache/equipment-guide/YYYY-MM-DD-coverage.json`에서 수집 과정에서 발견된 직업의 모든 전투력 구간별 유효 캐릭터 수와 30명 미만 구간을 확인할 수 있습니다. 부위별 표본은 빈 슬롯 때문에 더 적을 수 있습니다.
...
이번 실행 로그는 `.cache/equipment-guide/2026-09-10-collection.log`에 저장합니다. `eligible`은 재획 세팅 제외 후 유효 캐릭터 수, `groups`는 표본이 있는 직업·전투력 구간 수, `published`는 로컬 공개 스냅샷 반영 여부입니다. 배포 완료를 뜻하지 않습니다.
