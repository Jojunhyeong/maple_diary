...
export function Statistics({ stat, label, comparison }: { stat?: EquipmentGuideStat; label: string; comparison: string }) {
  ...
  return <div>
    ...
    <section className={styles.detailSection}><h3>많이 사용하는 아이템</h3><ol className={styles.ranking}>{stat.items.map(item =>
      <li key={item.itemName}><span>{item.itemName}</span><strong>{item.ratio}%</strong></li>)}</ol></section>
