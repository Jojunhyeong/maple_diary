...
import { COMBAT_BUCKETS, EQUIPMENT_SLOTS, findCombatBucket, type EquipmentSlotId, type EquipmentGuideDataset } from './model';
...
function CharacterEquipmentGuide({ profile }: { profile: LocalCharacterProfile }) {
  ...
  const ownBucket = findCombatBucket(profile.character_combat_power);
  const [bucket, setBucket] = useState(ownBucket?.id ?? '');
  const [selected, setSelected] = useState<EquipmentSlotId>('hat');
  const dialog = useRef<HTMLDialogElement>(null);
  const query = useQuery({
    queryKey: ['equipment-guide', job, bucket],
    enabled: !!job && !!bucket,
    queryFn: async ({ signal }) => {
      const response = await fetch('/api/equipment-guide?' + new URLSearchParams({ job, bucket }), { signal });
  ...
  const bucketLabel = COMBAT_BUCKETS.find(item => item.id === bucket)?.label ?? '전투력 구간 선택';
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.filters}>
      ...
      <label>전투력 구간<select value={bucket} onChange={event => setBucket(event.target.value)}><option value="" disabled>비교할 구간 선택</option>{COMBAT_BUCKETS.map(value => <option value={value.id} key={value.id}>{value.label}</option>)}</select></label>
      {ownBucket && <button onClick={() => setBucket(ownBucket.id)}>내 전투력 구간으로</button>}
    </div>
    <p className={styles.notice}>종합 랭킹에서 수집한 실제 착용 장비입니다. 아이템 드롭률·메소 획득량 잠재를 장착한 재획 세팅은 제외하며, 표본이 적은 구간은 사용 비율이 크게 달라질 수 있어요.</p>
    {!ownBucket && <p className={styles.notice}>내 전투력이 없거나 제공 범위(5천만~14억) 밖에 있어요. 비교할 전투력 구간을 직접 선택해 주세요.</p>}
    {!!bucket && query.isPending && <p role="status">실제 장비 통계를 불러오는 중이에요.</p>}
    {query.isError && <p role="alert">장비 통계를 불러오지 못했어요. <button onClick={() => query.refetch()}>다시 시도</button></p>}
    {!!bucket && !query.isPending && !query.isError && !data.stats.length && !query.data?.stale && <p role="status" className={styles.notice}>{job} · {bucketLabel} 구간의 표본을 아직 확보하지 못했어요. 아래에서 내 장비는 확인할 수 있어요.</p>}
    ...
    <div className={styles.workspace}>
      <section className={styles.inventory} aria-label="부위별 장비 통계">
        <div className={styles.panelTitle}><div><h2>많이 사용하는 장비</h2><p>{job} · {bucketLabel}</p></div><span>수집 표본 {stat?.sampleCount ?? 0}명<small>선택 부위 착용자 기준</small></span></div>
      ...
      <aside className={styles.statistics} aria-label="선택한 장비 상세 통계">{!query.isPending && !query.isError && <Statistics stat={stat} label={slot.label} bucket={bucketLabel} />}</aside>
    </div>
    <section className={styles.info}><h2>통계 안내</h2><p>동일 날짜의 Nexon Open API 전투력·착용 장비를 직업과 구간별로 집계합니다. 순위·레벨 기반 표본이므로 전체 유저의 장비 분포와 다를 수 있어요.</p><p>아이템 사용 비율은 해당 부위 착용자 기준이며, 강화·잠재 통계는 가장 많이 사용한 아이템의 착용자 기준입니다. 미수집 구간은 추정하지 않습니다.</p><p>Data based on NEXON Open API</p></section>
    ...
    <dialog ref={dialog} className={styles.dialog} aria-label={slot.label + ' 장비 통계'} onClick={event => { if (event.target === event.currentTarget) dialog.current?.close(); }}>
      <div className={styles.dialogBody}><button autoFocus className={styles.close} aria-label="장비 통계 닫기" onClick={() => dialog.current?.close()}>닫기 ✕</button><Statistics stat={stat} label={slot.label} bucket={bucketLabel} /></div>
    </dialog>
  </main>;
}
