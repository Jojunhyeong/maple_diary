...
import { EQUIPMENT_SLOTS, type EquipmentGuideLoadout, type EquipmentGuideLoadoutItem } from './model';
...
export function LoadoutCarousel({ loadouts, onSelectItem }: { loadouts?: EquipmentGuideLoadout[]; onSelectItem?: (loadout: EquipmentGuideLoadout, item: EquipmentGuideLoadoutItem) => void }) {
  ...
  return <section className={styles.loadouts} aria-labelledby="loadout-title">
    <div className={styles.loadoutTitle}>
      <div><p className={styles.eyebrow}>실제 캐릭터 조합</p><h2 id="loadout-title">추천 세팅</h2><p>실제 한 캐릭터가 함께 착용한 장비 조합이에요. 장비를 누르면 내 장비와 비교할 수 있어요.</p></div>
    ...
    <div className={styles.detailDots} aria-label="추천 세팅 선택">{loadouts.map((loadout, index) => <button type="button" key={loadout.id} aria-label={`추천 세팅 ${index + 1}`} aria-current={index === active ? 'true' : undefined} onClick={() => move(index)} />)}</div>
    <p className={styles.sample}>서로 다른 세트 구성을 우선해 최대 3개를 선정합니다. 각 카드는 실제 한 캐릭터의 조합이며, 여러 캐릭터의 인기 부위를 합친 가상 세팅이 아닙니다.</p>
  </section>;
}
